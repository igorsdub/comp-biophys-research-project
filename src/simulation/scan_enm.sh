#!/bin/bash
# Executes essential DDPT routines on a PDB file
# DDPT must be installed and added to local PATH variable

if [ "$#" -ne 3 ]; then
    echo "Usage: $0 <pdb-filepath> <results-filepath> <optional-flags>"
    echo "Example: $0 pdb/processed/1c3b.1.pdb data/raw/ "-mass -c 8.0""
    exit 1
fi
...
echo "Always presnet GENENMM flags: -pdb -ca -het"
...
cp -f $ROOT/tmp/{resmass.dat,cutoff.radius,res.force,fix.springs} -t $WORK_DIR
...
GENENMM -pdb $PDB_FILEPATH -ca -het $GENENMM_FLAGS
...
do
    echo "Running: RMSCOL for $MODE_NUM non-trivial mode/s"
